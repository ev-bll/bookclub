...
import {
  BookOpen, SlidersHorizontal, X, Star, Check,
  Plus, Search, ChevronLeft, ArrowRight, Trash2,
} from "lucide-react";
...
const FALLBACK_COLORS: [string, string][] = [
  ["#7A2E1A", "#B85A3A"], ["#1E4D2E", "#3D8C5A"],
  ["#3A2218", "#7A5232"], ["#5A2E4A", "#A06490"],
  ["#6B4A0A", "#C49528"], ["#1A2848", "#2E4A8A"],
  ["#3A2010", "#7A4A28"], ["#102038", "#1A4870"],
];

const SUGGESTIONS = ["El nombre del viento", "Circe", "Dune", "Rebecca", "Sapiens"];
...
const SUBJECT_TRANSLATIONS: [RegExp, string][] = [
  [/fiction/i, "Ficción"],
  [/non.?fiction/i, "No ficción"],
  [/novel/i, "Novela"],
  [/short stor/i, "Cuentos"],
  [/horror|terror/i, "Terror"],
  [/romance/i, "Romance"],
  [/thriller/i, "Thriller"],
  [/mystery|detective/i, "Misterio"],
  [/crime/i, "Crimen"],
  [/fantasy/i, "Fantasía"],
  [/science fiction|sci.fi/i, "Ciencia ficción"],
  [/adventure/i, "Aventura"],
  [/historical/i, "Histórico"],
  [/biography|memoir/i, "Biografía"],
  [/autobiography/i, "Autobiografía"],
  [/self.help/i, "Autoayuda"],
  [/poetry|poems/i, "Poesía"],
  [/children|juvenile/i, "Infantil"],
  [/young adult/i, "Juvenil"],
  [/graphic novel/i, "Novela gráfica"],
  [/comic/i, "Cómic"],
  [/philosophy/i, "Filosofía"],
  [/psychology/i, "Psicología"],
  [/history/i, "Historia"],
  [/politics/i, "Política"],
  [/economics/i, "Economía"],
  [/science/i, "Ciencia"],
  [/nature/i, "Naturaleza"],
  [/travel/i, "Viajes"],
  [/cooking|food/i, "Gastronomía"],
  [/art/i, "Arte"],
  [/music/i, "Música"],
  [/sport/i, "Deporte"],
  [/war/i, "Guerra"],
  [/love/i, "Amor"],
  [/family/i, "Familia"],
  [/coming.of.age/i, "Iniciática"],
  [/dystop/i, "Distopía"],
  [/utop/i, "Utopía"],
  [/mythology/i, "Mitología"],
  [/magic/i, "Magia"],
  [/supernatural/i, "Sobrenatural"],
  [/gothic/i, "Gótico"],
  [/classic/i, "Clásico"],
  [/literature/i, "Literatura"],
  [/drama/i, "Drama"],
  [/humor|comedy/i, "Humor"],
  [/satire/i, "Sátira"],
  [/religion|spiritual/i, "Espiritualidad"],
  [/lgbtq|queer/i, "LGBTQ+"],
  [/feminist/i, "Feminismo"],
];
...
function fromOLDoc(doc: OLDoc, idx: number): Book {
  const id       = doc.key.replace("/works/", "");
  const title    = doc.title ?? "Sin título";
  const words    = title.split(/\s+/);
  const short    = words.length <= 2 ? title : words.slice(0, 2).join(" ");
  ...
  const genre    = rawGenre.length > 22 ? rawGenre.slice(0, 20) + "…" : rawGenre;
  const colors   = FALLBACK_COLORS[idx % FALLBACK_COLORS.length];
  return {
    id, title, short,
    author:    doc.author_name?.[0] ?? "Autor desconocido",
    coverUrl, c1: colors[0], c2: colors[1],
...
async function fetchBooks(query: string): Promise<Book[]> {
  ...
  return (data.docs ?? []).filter(d => d.title).map(fromOLDoc);
}
...
function piePath(i: number, n: number, R: number, cx: number, cy: number): string {
  const sdeg = 360 / n;
  const s = -90 + i * sdeg, e = s + sdeg;
...
function winnerFromRot(rot: number, books: Book[]): Book {
  const n    = books.length;
  const sdeg = 360 / n;
  const eff  = (360 - ((rot % 360 + 360) % 360)) % 360;
  return books[Math.floor(eff / sdeg) % n];
}

// ─── StatusBar ────────────────────────────────────────────────────────────────

// ─── CoverImage — OL cover with Google Books fallback ────────────────────────

async function resolveGoogleBooks(title: string, author: string): Promise<string | null> {
  try {
    ...
    for (const item of (data.items ?? [])) {
      ...
      if (raw) return (raw as string).replace("http:", "https:").replace("zoom=1", "zoom=2");
    }
    return null;
  } catch {
    return null;
  ...
  return (
    <img
      ...
      onError={async e => {
        ...
        const gb = await resolveGoogleBooks(book.title, book.author);
        setSrc(gb);
      }}
    />
  );
  ...
  useEffect(() => {
    ...
    fetch(`https://openlibrary.org/works/${book.id}.json`)
      ...
      .then(async data => {
        ...
        const raw = typeof d === "string" ? d : typeof d?.value === "string" ? d.value : null;
        if (!raw) { setDesc(null); return; }
        try {
          const url = `https://translate.googleapis.com/translate_a/single?client=gtx&sl=en&tl=es&dt=t&q=${encodeURIComponent(raw)}`;
          const tr = await fetch(url).then(r => r.json());
          const translated = tr[0]?.map((s: [string]) => s[0]).join("") ?? raw;
          setDesc(translated);
        } catch {
          setDesc(raw);
        }
...
function SearchSheet({ library, onAdd, onRemove, onViewDetail, onClose }: {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      try { setResults(await fetchBooks(query)); }
      catch { setResults([]); }
...
function LibraryScreen({ library, onAdd, onRemove, onViewDetail, onGoToWheel }: {
  library: Book[];
  onAdd: (b: Book) => void;
  onRemove: (id: string) => void;
...
function WheelScreen({ library, activeIds, rotation, spinning, onSpin, onBack, onToggleActive, onSelectAll, onSelectNone, winner, showWinner, onCloseWinner }: {
...
export default function App() {
  const [library, setLibrary] = useState<Book[]>(() => {
    try { return JSON.parse(localStorage.getItem("mpl-library") ?? "[]") as Book[]; }
    catch { return []; }
  });
  useEffect(() => { localStorage.setItem("mpl-library", JSON.stringify(library)); }, [library]);

  const [activeIds, setActiveIds] = useState<Set<string>>(() => {
    try { return new Set(JSON.parse(localStorage.getItem("mpl-active") ?? "[]") as string[]); }
    catch { return new Set(); }
  });
  useEffect(() => { localStorage.setItem("mpl-active", JSON.stringify([...activeIds])); }, [activeIds]);
  ...
  const addBook = useCallback((book: Book) => {
    setLibrary(prev => prev.some(b => b.id === book.id) ? prev : [...prev, book]);
    setActiveIds(prev => new Set([...prev, book.id]));
  }, []);

  const removeBook = useCallback((id: string) => {
    setLibrary(prev => prev.filter(b => b.id !== id));
    setActiveIds(prev => { const n = new Set(prev); n.delete(id); return n; });
    setRotation(0); setShowWinner(false);
  }, []);
  ...
  const selectAll  = useCallback(() => { setActiveIds(new Set(library.map(b => b.id))); setRotation(0); setShowWinner(false); }, [library]);
  const selectNone = useCallback(() => { if (activeIds.size > 1 && library.length > 0) { setActiveIds(new Set([library[0].id])); setRotation(0); setShowWinner(false); } }, [activeIds, library]);

  const goToWheel   = useCallback(() => { setDir(1);  setScreen("wheel"); }, []);
  const goToLibrary = useCallback(() => { setDir(-1); setScreen("library"); setShowWinner(false); setSpinning(false); }, []);

  const spin = useCallback(() => {
    if (spinning || activeBooks.length < 2) return;
    const extra = 1800 + Math.floor(Math.random() * 1080);
    const next  = rotation + extra;
    setRotation(next); setSpinning(true); setShowWinner(false);
    setTimeout(() => {
      setSpinning(false);
      setWinner(winnerFromRot(next, activeBooks));
      setShowWinner(true);
    }, 4400);
  }, [spinning, rotation, activeBooks]);
    ...
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "linear-gradient(145deg, #DDD0BC 0%, #C8B89E 45%, #D5C5AD 100%)", padding: 16 }}>
      <div style={{ position: "relative", width: 390, height: 844, overflow: "hidden", background: "#F8F2E5", borderRadius: 44, boxShadow: "0 40px 100px rgba(44,26,14,0.35), 0 0 0 1px rgba(139,100,70,0.15), inset 0 0 0 1px rgba(255,255,255,0.45)" }}>
        <AnimatePresence initial={false} custom={dir} mode="wait">
          {screen === "library" ? (
            <motion.div key="library" custom={dir} variants={slideVariants} initial="enter" animate="center" exit="exit"
            ...
            <motion.div key="wheel" custom={dir} variants={slideVariants} initial="enter" animate="center" exit="exit"
              transition={{ type: "spring", damping: 32, stiffness: 300 }}
              style={{ position: "absolute", inset: 0, zIndex: 2 }}>
              <WheelScreen
                library={library}
